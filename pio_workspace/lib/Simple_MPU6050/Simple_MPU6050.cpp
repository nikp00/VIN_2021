...
Simple_MPU6050 &Simple_MPU6050::full_reset_fifo(void)
{ // Official way to reset fifo
...
{
	if (CheckInterrupt && !CheckForInterrupt())
		return *this;
	...
	if (on_FIFO_cb)
		on_FIFO_cb(gyro, accel, quat, sensor_timestamp);
	return *this;
}
 ...
 * ================================================================ */
int8_t Simple_MPU6050::GetCurrentFIFOPacket(uint8_t *data, uint8_t length)
{ // overflow proof
	int16_t fifoC;
...
{
	if (length == 1)
		I2CReadCount = readBit(AltAddress, regAddr, bitNum, Data);
	else
		I2CReadCount = readBits(AltAddress, regAddr, bitNum, length, Data);
	return *this;
}
// MPUi2cReadBytes
...
#define CompassCheck(Cnt)                                \
	{                                                    \
		uint8_t D;                                       \
		Serial.print(F("\n"));                           \
		Serial.print(Cnt);                               \
		Serial.print(F(" Read AKM Who am I: "));         \
		Serial.print(I2Cdev::readBytes(0x0C, 0, 1, &D)); \
...
#ifdef interruptPin
	attachInterrupt(
		digitalPinToInterrupt(interruptPin), [] { mpuInterrupt = true; }, RISING); //NOTE: "[]{mpuInterrupt = true;}" Is a complete funciton without a name. It is handed to the callback of attachInterrupts Google: "Lambda anonymous functions"
#endif
...
#define LOAD_CHUNK (16)
	...
	if (loaded)
		return *this; /* DMP should only be loaded once. */
	if (!firmware)
		*this;
...
void Simple_MPU6050::view_Vital_MPU_Registers()
{
	...
	Serial.print(F("writeByte(devAddr,0x1C, 0x"));
	readBytes(0x68, 0x1C, 1, &val);
	Serial.print((val >> 4), HEX);
	Serial.print((val & 0x0F), HEX);
	Serial.println(F(");/n")); // ACCEL_CONFIG:
	Serial.print(F("writeByte(devAddr,0x37, 0x"));
	readBytes(0x68, 0x37, 1, &val);
	Serial.print((val >> 4), HEX);
	Serial.print((val & 0x0F), HEX);
	Serial.println(F(");/n")); // INT_PIN_CFG:
	Serial.print(F("writeByte(devAddr,0x6B, 0x"));
	readBytes(0x68, 0x6B, 1, &val);
	Serial.print((val >> 4), HEX);
	Serial.print((val & 0x0F), HEX);
	Serial.println(F(");/n")); // PWR_MGMT_1:
	Serial.print(F("writeByte(devAddr,0x19, 0x"));
	readBytes(0x68, 0x19, 1, &val);
	Serial.print((val >> 4), HEX);
	Serial.print((val & 0x0F), HEX);
	Serial.println(F(");/n")); // SMPLRT_DIV:
	Serial.print(F("writeByte(devAddr,0x1A, 0x"));
	readBytes(0x68, 0x1A, 1, &val);
	Serial.print((val >> 4), HEX);
	Serial.print((val & 0x0F), HEX);
	Serial.println(F(");/n")); // CONFIG:
	...
	Serial.print(F("writeByte(devAddr,0x1B, 0x"));
	readBytes(0x68, 0x19, 1, &val);
	Serial.print((val >> 4), HEX);
	Serial.print((val & 0x0F), HEX);
	Serial.println(F(");/n"));							// GYRO_CONFIG:
...
#define DPRINTHEX(Num)           \
	Serial.print(Num >> 4, HEX); \
	Serial.print(Num & 0X0F, HEX);
#define ShowByte(Addr)                          \
	{                                           \
		uint8_t val;                            \
		I2Cdev::readBytes(0x68, Addr, 1, &val); \
		Serial.print("0x");                     \
		DPRINTHEX(Addr);                        \
		Serial.print(" = 0x");                  \
		DPRINTHEX(val);                         \
...
Simple_MPU6050 &Simple_MPU6050::ConvertToRadians(float *xyz, float *ypr)
{
	...
	for (int i = 0; i < 3; i++)
		ypr[i] = xyz[i] * degrees_to_radians;
	return *this;
}
...
#define SPRINTHEX(Num)    \
	print(Num >> 4, HEX); \
	Serial.print(Num & 0X0F, HEX); // Prints 2 Digits even when Value < 16
	...
	delay(100);
	if (!HIGH_SENS)
		mRes = 10. * 4912. / 8190.; // Proper scale to return milliGauss MFS_14BITS
	else
		mRes = 10. * 4912. / 32760.0; // Proper scale to return milliGauss MFS_16BITS
	uint8_t AKMData[3];
...
#define DPRINTBINL(Num)                         \
	for (int i = 0; i < (sizeof(Num) * 8); i++) \
		Serial.write(((Num >> i) & 1) == 1 ? '1' : '0'); // Prints a binary number with following Placeholder Zeros  (Automatic Handling)
#define DPRINTBINLX(S, Num, nl)                          \
	Serial.print(F(S));                                  \
	for (int i = 0; i < (sizeof(Num) * 8); i++)          \
		Serial.write(((Num >> i) & 1) == 1 ? '1' : '0'); \
	if (nl)                                              \
		Serial.println(); // Prints a binary number with following Placeholder Zeros  (Automatic Handling)
		int8_t Num;
...
#define ShowValue(Name, FunctionD) \
	FunctionD;                     \
	Serial.print(Name);            \
	Serial.print(" = 0x");         \
	DPRINTHEX(D);                  \
